 ...
 *        
 */
class C123SMSSender implements ISMSSender {
	...
	protected function addSendRecord ($phonenum, $content, $retInfo) {
		...
		for ($i = 0; $i < count($phonenumArr); ++ $i) {
			...
			$record->status = 1 == $retInfo[0] ? SMSSendRecord::STATUS_SEND_SUC : SMSSendRecord::STATUS_RECEIVED_FAILED;
			$record->gatewayError = $retInfo[0];
			$record->msgId = $retInfo[1][0];
