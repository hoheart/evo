...
// KCAPTCHA is a free software. You can freely use it for developing own site or
// software.
// If you use this software as a part of own sofware, you must leave copyright
// notices intact or add KCAPTCHA copyright notices to own.
// As a default configuration, KCAPTCHA has a small credits text at bottom of
// CAPTCHA image.
...
class KCAPTCHA {
	...
	function KCAPTCHA ($str, $captchaLen) {
		...
		do {
			...
			// }
			// if(!preg_match('/cp|cb|ck|c6|c9|rn|rm|mm|co|do|cl|db|qp|qb|dp|ww/',
			// $this->keystring)) break;
			$this->keystring = $str;
			// }
			...
			for ($i = 0; $i < $length; $i ++) {
				...
				$y = (($i % 2) * $fluctuation_amplitude - $fluctuation_amplitude / 2) * $odd + mt_rand(
						- round($fluctuation_amplitude / 3), round($fluctuation_amplitude / 3)) +
						...
						for ($sy = 3; $sy < $fontfile_height - 10; $sy += 1) {
							for ($sx = $m['start'] - 1; $sx < $m['end']; $sx += 1) {
								...
								if ($opacity < 127) {
									...
									if ($py >= $height || $py < 0 )
										break;
									for ($px = min($left, $width - 1); $px > $left - 160 && $px >= 0; $px -= 1) {
				...
				imagecopy($img, $font, $x - $shift, $y, $m['start'], 1, $m['end'] - $m['start'], 
						$fontfile_height);
		...
		imagestring($img2, 2, $width / 2 - imagefontwidth(2) * strlen($credits) / 2, $height - 2, 
				$credits, $background);
